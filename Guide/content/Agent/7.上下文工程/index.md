...
1. **大海捞针问题（Needle In A Haystack）：** 上下文越长，模型越容易产生“注意力分散”或“中间失忆（Lost in the Middle）”
    
2. **Token 成本与延迟飙升：** 每次请求传输和处理海量 Token，不仅 API 费用成倍增加，首次 Token 输出延迟（TTFT）也很长。
...
- 分块递归摘要（Recursive Summarization）**处理长文档**：
	- 采用大数据的 **Map-Reduce（分治法）** 思路。
	- **第一层（Map）**：将 10 万字文档切成 10 块，让小模型对每个小块分别生成 200 字摘要。
	- **第二层（Reduce）**：把这 10 个摘要拼在一起，再让模型二次提炼，最终生成一份 500 字的全局核心骨架。
        
- 滑动窗口与增量记忆状态机（Incremental Memory）**处理对话历史**：
	1. **热记忆（滑动窗口）：** 始终保留最新的 N 轮原始对话（比如 5 轮）。这保证了 AI 能够精确理解当前的指代关系和聊天语气。
	2. **冷记忆（增量更新）：**
