...
1. 任务不会因为进程重启而丢失：任务持久化在SQLite
2. 处理中断的任务能够重新执行：相册进程启动时，会把遗留的 processing 任务恢复成 pending，协调器之后重新领取并执行。
3. 恢复后不会被旧任务污染：每次领取都会生成新的 claim_token。假设旧 worker 在超时/重启边界下迟到返回，它持有旧 token，因此不能覆盖新 worker 的执行结果。
