...
一开始其实并没有确定是内存问题，而是先排除了业务逻辑错误。因为日志没有任何异常，程序也没有 panic，说明不是代码直接报错。

同时进行操作时，日志也没有打印请求，怀疑是资源瓶颈，比如内存、cpu等。
...
有一点增加，但不明显。 因为分块后 Document 数量变多，元数据可能增多。不过 Term Dictionary 基本不变，Posting List 又采用压缩存储，所以整体索引体积增长有限。我们实际测试中 HeapInuse 降低了约 76%，因此这是一个值得的权衡
