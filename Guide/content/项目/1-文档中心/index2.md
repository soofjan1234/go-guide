...
评估不只看总耗时，还看索引吞吐、内存峰值、磁盘占用、失败或 OOM 情况，以及检索结果是否正确
...
不过 Posting List 又采用压缩存储，所以整体索引体积增长有限，大概10-20%。我们实际测试中 HeapInuse 降低了约 76%，因此这是一个值得的权衡
