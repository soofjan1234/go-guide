...
Transformer 引入了多头注意力——相当于同时派出了多组独立的“侦探小分队”：

- 第一组小队：专门关注语法指代关系（比如“它”对应“苹果”）。
- 第二组小队：专门关注空间/时间关系（比如“把...递给...”的动作关联）。
- 第三组小队：专门关注修饰关系（比如“甜”对应“苹果”）。
...
最擅长任务：判别式与理解类任务：
- 文本分类 / 情感分析（如垃圾邮件识别、用户评论打分）。
- 命名实体识别 (NER) / 词性标注。
- 文本向量化 (Text Embedding) / 句抽取式问答（如向量检索、RAG 的 Embedding 阶段）
