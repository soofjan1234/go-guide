...
3. Purge Thread：提交后旧版本留在 undo，Purge 等到最小活跃视图不再引用才物理删除。
...
InnoDB 引入了一个额外的存储层：
- 留底（快写）：在脏页真正写入 .ibd 数据文件前，先将其复制到内存的 Doublewrite Buffer。随后以大块顺序写的方式，刷入磁盘上独立的双写文件（形如 #ib_16384_0.dblwr，MySQL 8.0.20 之前存放在系统表空间 ibdata1）
- 干活（慢写）：确认“备胎”写完毕后，再将脏页按本来的方式离散地写入真正的表数据 .ibd 文件中。

3. 自救表现

- 情况 A： 如果在写“双写文件”时断电，数据页还没动，直接用该页+redolog重做。
- 情况 B： 如果在写“正式数据文件”时断电，导致页损坏。会去双写文件里找到那个完整的副本。
