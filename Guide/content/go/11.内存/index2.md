...
1. 极致减轻 GC 压力
    - 大部分临时变量、局部变量生命周期仅在函数内部。逃逸分析把它们全部按在栈上分配。
    - 函数执行完直接移指针销毁，根本不需要 GC 介入。
2. 保障内存安全
    - 当开发者在 Go 里写出“返回局部变量指针”的代码时，编译器自动分析出它逃逸了，默默将其分配到堆上
3. 提升 CPU 缓存局部性
    - 栈内存是连续的高速缓存行（L1/L2 Cache 友好），栈上分配的数据访问速度远高于离散分布在堆内存中的对象。
...
2. 阶段 I：准备（STW），**找根节点**
3. 阶段 II：并发标记（并发），**并发标记**
4. 阶段 III：标记收尾（STW），**刷写屏障缓冲**（wbBuf）、做终止检查
5. 阶段 IV：并发清扫（并发），**并发清理**
...
Go 1.26：Green Tea GC 主要优化 Go GC 的标记扫描过程。

- 以前按对象扫描，容易在不同内存位置之间跳来跳去；
- 它把任务按内存页组织，尽量集中扫描同一页里已发现的存活对象，提高 CPU 缓存命中率，降低 GC 的 CPU 开销。

**对象是否存活仍然按指针可达性判断，变化的是扫描工作的组织方式。**
