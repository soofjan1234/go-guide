...
\begin{itemize}
    ...
    \item \textbf{中间件与工程基础}
    \begin{itemize}
        \item 熟悉 RabbitMQ 消息队列，理解消息重复、丢失处理方法；了解 Kafka、RocketMQ 相关选型。
        \item 熟悉计算机网络核心协议，掌握 HTTP/HTTPS、NAT、DNS、TCP/UDP 等基础机制。
        \item 熟悉 Docker 容器化部署技术，了解 k8s 架构。
        \item 了解/关注 AI Agent 相关技术栈，包括 Transformer、Decoder-Only 架构、智能体范式、LangChain 与 LangGraph
    \end{itemize}
...
\noindent\textcolor{gray}{\textbf{技术栈：} Go, Gin, Gorm, MySQL, SQLite, Bleve, P2P, Docker, Vue, RKNN, OpenCV}
}
\vspace{0.3ex}
\vspace{0.3ex}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item 针对内存受限环境，完成了从 Elasticsearch 到 Bleve 的搜索引擎迁移，实现快速的全文与多维度检索，约 4 倍查询性能提升。
    ...
    \begin{itemize}
        ...
        \item Docker：封装 Docker Engine API 实现镜像、容器、网络、挂载和运行状态管理。
...
\section{\faCode\ 个人项目}
\paragraph*{视频 Feed 流系统}\mbox{}\\
{\small
\noindent 基于 Go 开发的分布式 Feed 流服务，实现用户动态发布、关注关系、时间线查询等功能，采用缓存、异步消息和 Feed 分发策略提升系统吞吐能力。\\
\noindent\textcolor{gray}{\textbf{技术栈：} Go, React, Redis, RabbitMQ, MySQL}
}
\vspace{0.3ex}

\begin{itemize}
    \item \textbf{Feed 性能优化}：实现时间线、关注流、推荐流、热榜流，使用游标分页、缓存和批量查询提升 Feed 读取速度。
    \item \textbf{推拉结合视频分发}：普通作者发布走粉丝推送，大 V 作者发布走拉取合并，解决大 V 发布带来的海量 fanout 压力。
    \item \textbf{异步化}：引入 RabbitMQ 消息队列解耦动态发布和 Feed 扩散流程，提升写入接口响应速度，并通过消费者幂等机制避免重复消费。
\end{itemize}
