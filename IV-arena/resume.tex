...
\noindent\textcolor{gray}{\textbf{技术栈：} Go, Gin, Gorm, MySQL, PostgreSQL, SQLite, Bleve, gRPC, Docker, RKNN, LangGraph}
}
\vspace{0.3ex}
\vspace{0.3ex}
...
\begin{itemize}
    ...
    {\small
    \noindent\textcolor{gray}{负责 NAS 售后英文邮件处理：真实收件后检索产品文档与已审核案例，生成带引用的回复草稿，人工审核通过后再发出。}
    }
    \vspace{0.2ex}
    \begin{itemize}
        \item 用 LangGraph + PostgreSQL Checkpointer 编排分类、检索改写、证据评估、生成校验与人工 interrupt；出草稿 P95 约 31s，审核通过才出站。
        \item 产品文档与历史案例分路检索（全文 + pgvector，RRF 融合）；依据不足、资料冲突或高风险时只补信息或转人工。上线 1 个月直接批准 46\%、编辑后批准 37\%、拒绝 7\%、转人工 10\%。
    \end{itemize}
    \vspace{1ex}
    ...
    \vspace{0.2ex}
    \begin{itemize}
        \item 基于拉普拉斯方差实现照片模糊检测；通过 CPU 自适应 Worker Pool 并行计算，SQLite 串行写入并支持取消，吞吐提升 65.3\%。
        \item 实现可恢复的 AI 异步任务协调器：任务落库，用 claim token 领取；进程重启或旧 worker 迟到时，不能覆盖新结果。
    \end{itemize}
    \vspace{1ex}
    ...
    \begin{itemize}
        ...
        \item 基于 Protobuf/gRPC over Unix Domain Socket 对本机业务提供接口；设计 NPU 租约调度机制，支持优先级、deadline 及心跳超时回收。
    \end{itemize}
    \vspace{1ex}
